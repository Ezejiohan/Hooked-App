...
const sendEmail = async (options) => {
    ...
    const transporter = nodemailer.createTransport({
        ...
        auth: {
            user: process.env.APP_EMAIL, // The email address used to send emails (stored in environment variable)
            pass: process.env.APP_PASSWORD // The password or app-specific password for the email account (stored in environment variable)
        }
    });
    ...
    let mailOptions = {
        from: {
            name: "Hooked-App", // The name displayed as the sender
            address: process.env.APP_EMAIL // The email address displayed as the sender
        },
